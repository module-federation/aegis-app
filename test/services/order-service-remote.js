...
const { RestOrderAdapter } = require('../../src/adapters/order-adapter');

const OrderService = require('../../src/services/order-service').OrderService;

const url = 'http://localhost:8070/api/orders/'
...
(async () => {
  ...
  const orderId = (await os.addOrder(orderInfo)
    .addOrderItem('item1', 2343.99)
    .createOrder())
    .then(os => os.orderId);
